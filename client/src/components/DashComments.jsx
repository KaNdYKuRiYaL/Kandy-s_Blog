...
export default function DashComments() {
  ...
  const handleDeleteComment = async()=>{
    ...
    try{
      ...
      const data = await res.json();
      if(res.ok){
        setComments((prev) => prev.filter((comment)=> comment._id !== commentIdDelete ));
        setShowModal(false);
      }
      else{
        ...
        <>
          <Table hoverable className='shadow-md'>
            ...
            {comments.map((comment)=>(
              <Table.Body className='divide-y text-md' key={comment._id}>
        ...
        size='md'>
          <Modal.Header/>
            <Modal.Body>
              ...
              <h3 className='mb-5 text-lg text-gray-500 dark:text-gray-400'>Are your sure you want to delete this comment?</h3>
